...
class TestUserDAOInsert:
    def test_insert_one_creates_document(self, app: Flask, mongo_db: Database, sample_user: dict[str, Any]) -> None:
    ...
    def test_insert_one_returns_insert_result(self, app: Flask, mongo_db: Database, sample_user: dict[str, Any]) -> None:
        mongo_db.users.delete_many({})
    ...
    def test_insert_multiple_documents(self, app: Flask, mongo_db: Database, sample_users: list[dict[str, Any]]) -> None:
        mongo_db.users.delete_many({})
...
class TestUserDAOFind:
    ...
    def test_find_returns_all_documents(self, app: Flask, inserted_users: list[dict[str, Any]]) -> None:
        result = UserDAO.find()

        assert len(result) == len(inserted_users)

    def test_find_returns_parsed_documents(self, app: Flask, inserted_users: list[dict[str, Any]]) -> None:
        result = UserDAO.find()
...
class TestUserDAOFindOneById:
    def test_find_one_by_id_returns_document(self, app: Flask, inserted_user: dict[str, Any]) -> None:
    ...
    def test_find_one_by_id_accepts_string_id(self, app: Flask, inserted_user: dict[str, Any]) -> None:
        result = UserDAO.find_one_by_id(inserted_user["_id"])

        assert result is not None
        assert isinstance(result["_id"], str)

    def test_find_one_by_id_returns_all_fields(self, app: Flask, inserted_user: dict[str, Any]) -> None:
        result = UserDAO.find_one_by_id(inserted_user["_id"])
...
class TestUserDAOFindOneByUsername:
    def test_find_one_by_username_returns_document(self, app: Flask, inserted_user: dict[str, Any]) -> None:
        result = UserDAO.find_one_by_username(inserted_user["username"])

        assert result is not None
        assert result["username"] == inserted_user["username"]

    def test_find_one_by_username_returns_none_for_nonexistent(self, app: Flask, mongo_db: Database) -> None:
        mongo_db.users.delete_many({})
    ...
    def test_find_one_by_username_is_case_sensitive(self, app: Flask, inserted_user: dict[str, Any]) -> None:
...
class TestUserDAOUpdateOneByUsername:
    def test_update_one_by_username_updates_document(self, app: Flask, inserted_user: dict[str, Any], mongo_db: Database) -> None:
        new_score = 999

        UserDAO.update_one_by_username(inserted_user["username"], {"total_score": new_score})

        doc = mongo_db.users.find_one({"username": inserted_user["username"]})
        assert doc["total_score"] == new_score

    def test_update_one_by_username_returns_update_result(self, app: Flask, inserted_user: dict[str, Any]) -> None:
        result = UserDAO.update_one_by_username(inserted_user["username"], {"total_score": 500})

        assert isinstance(result, UpdateResult)
        assert result.acknowledged is True

    def test_update_one_by_username_modifies_only_specified_fields(self, app: Flask, inserted_user: dict[str, Any], mongo_db: Database) -> None:
    ...
    def test_update_one_by_username_nonexistent_returns_zero_matched(self, app: Flask, mongo_db: Database) -> None:
        mongo_db.users.delete_many({})
    ...
    def test_update_one_by_username_updates_nested_scores(self, app: Flask, inserted_user: dict[str, Any], mongo_db: Database) -> None:
        new_scores = {"General": 500, "Normal": 300, "Hard": 200}

        UserDAO.update_one_by_username(inserted_user["username"], {"scores": new_scores})
...
class TestUserDAODelete:
    def test_delete_one_by_id_removes_document(self, app: Flask, inserted_user: dict[str, Any], mongo_db: Database) -> None:
    ...
    def test_delete_one_by_id_returns_delete_result(self, app: Flask, inserted_user: dict[str, Any]) -> None:
        result = UserDAO.delete_one_by_id(inserted_user["_id"])
